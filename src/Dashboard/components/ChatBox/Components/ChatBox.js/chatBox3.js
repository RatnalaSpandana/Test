...
function Chatbox3( {setSelectedimg} ){

    const { docs  } = UseFirestore('Spandana');
    const [msg, setMsg] =  useState('');
    const [username, setUsername] = useState('')
    const [to, setTo] = useState('')

    let userName = 'Spandana'
           ...
           
            {
                docs && docs.map(doc => (
                    ...
                    <div>
                        ...
                        {
                (doc.userName === userName || doc.Recciptant === userName) ?

              <div><span style={{color:'black'}}>{doc.userName}:</span>   
